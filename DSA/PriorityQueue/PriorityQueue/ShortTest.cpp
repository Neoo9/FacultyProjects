...
void testIncreasePriority() {
	...
	pq.push(2, 2);
	pq.push(12, 12);
	assert(pq.increasePriority(10, 9) == 10);
	assert(pq.increasePriority(12, 2) == 12);
	assert(pq.top().first == 12);
	assert(pq.top().second == 2);
	assert(pq.increasePriority(80, 1) == -1);
	assert(pq.increasePriority(12, 1) == 2);
	assert(pq.top().first == 12);
	assert(pq.top().second == 1);
	try {
		pq.increasePriority(5, 20);
		assert(false);
	}
	catch (...) {
		assert(true);
